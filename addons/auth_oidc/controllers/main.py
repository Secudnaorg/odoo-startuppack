...
# Patched for Werkzeug 3.x: url_decode/url_encode removed upstream.
...
def _url_decode(qs):
    """Parse a query string into a flat dict (Werkzeug 3.x compat)."""
    return dict(parse_qsl(qs, keep_blank_values=True))


def _url_encode(d):
    """Encode a dict to a query string (Werkzeug 3.x compat)."""
    return urlencode(d)
    ...
    def list_providers(self):
        ...
        for provider in providers:
            ...
            if flow in ("id_token", "id_token_code"):
                params = _url_decode(provider["auth_link"].split("?")[-1])
                # nonce
                ...
                provider["auth_link"] = "{}?{}".format(
                    provider["auth_endpoint"], _url_encode(params)
                )
        return providers
